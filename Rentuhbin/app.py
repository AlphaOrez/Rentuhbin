...
import cv2
import numpy as np
import supervision as sv  # pylint: disable=no-name-in-module
import uvicorn
import torch
...
def draw_rectangle(
    ...
    # object_height_pixels =  int(object_height_meters * (image_height_pixels / 45))
    # object_width_pixels = int(object_width_meters * (image_width_pixels / 81.8))
...
def process_image(org_image, org_image_cv2: np.ndarray) -> str:
    ...
    home = os.getcwd()
    # existing_segmented_image_path = os.path.join(HOME, "data", "modified_image.png")
...
async def process_image_endpoint(
    ...
    # Save the path of the uploaded image for processing
    existing_segmented_image_path = os.path.join(home, "data", org_image_name)
    ...
    if os.path.isfile(existing_segmented_image_path):
        ...
        # Using the existing segmented image to draw the rectangle on the uploaded image
